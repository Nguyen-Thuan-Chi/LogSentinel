...
function Test-SysmonInstalled {
    ...
    try {
        # Check for Sysmon service (should be named "Sysmon")
        $sysmonService = Get-Service -Name Sysmon -ErrorAction SilentlyContinue
        
        if ($null -ne $sysmonService) {
            Write-Host "      ? Sysmon service found: $($sysmonService.Status)" -ForegroundColor Green
            ...
            if ($sysmonService.Status -eq "Running") {
                ...
                try {
                    $logExists = Get-WinEvent -ListLog Microsoft-Windows-Sysmon/Operational -ErrorAction Stop
                    Write-Host "      ? Sysmon event log is available" -ForegroundColor Green
                    Write-Host "        Log enabled: $($logExists.IsEnabled)" -ForegroundColor Gray
                    Write-Host "        Record count: $($logExists.RecordCount)" -ForegroundColor Gray
                    return $true
                }
                catch {
                    Write-Host "      ? Sysmon event log NOT available" -ForegroundColor Red
                    Write-Host "        Sysmon may not be properly configured" -ForegroundColor Yellow
                ...
                Write-Host "        Try: Start-Service -Name Sysmon" -ForegroundColor Yellow
                return $false
            }
        } else {
            ...
            Write-Host "        Then run Fix-SysmonConfig.ps1 to configure it" -ForegroundColor Yellow
            return $false
        }
    }
    catch {
